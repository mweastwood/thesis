...
function plot_and_label(z, y, icon, label; text_position=y[end])
    ...
    text(z[end]+0.3, text_position, label,
         color="white",
...
function shade_signal_region()
    ...
    y1 = [ 0,  0,   0,   0]
    y2 = [10, 10, 100, 100]
    fill_between(x, y1, y2, color="white", alpha=0.15, lw=0, zorder=0)
end
...
function go()
    ...
    plot_and_label(8.6, 248, "v", L"\textrm{Paciga et al. 2013}", text_position=300)
    plot_and_label(7.1, 164, "v", L"\textrm{Beardsley et al. 2016}")
    plot_and_label(18.4, 1e4, "wv", L"\textrm{this thesis}")
    ...
    text(11, 2, L"\textrm{Epoch of Reionization}",
         fontsize=20, horizontalalignment="center", color="white")
    text(19, 2, L"\textrm{Cosmic Dawn}",
         fontsize=20, horizontalalignment="center", color="white")
